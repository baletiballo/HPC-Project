...
public:
	...
	/*
		...
		Ausgabe: Das Tupel (loss, correct, filter_gradients, conv_bias_gradients, weight_gradient, conn_bias_gradient) 
	*/
	tuple<float, bool, vector<vector<vector<vector<float>>>>, vector<vector<float>>, vector<vector<float>>, vector<float>> forward
			(vector<vector<vector<float>>> &image, int_fast8_t label) {
	...
	tuple<float, int_fast8_t> learn
		...
		tuple<float, bool, vector<vector<vector<vector<float>>>>, vector<vector<float>>, vector<vector<float>>, vector<float>> Result = 
				forward(image,	label);
		...
		for (int i = 1; i < batchSize; i++) {
			...
			addGradients(get<2>(Result), get<3>(Result), get<4>(Result), get<5>(Result), filterGradients, filterBiases, weightGradient,
					weightBiases);
		}
