...
const int infaltionFactor = 4; //Zwischen zwei Originalpixel werden (inflationFaktor-1) interpoliert für die Trainingsdaten
...
const int imageSizeX_afterConvolution = imageSizeX - conv_size1 + 1; 
const int imageSizeY_afterConvolution = imageSizeY - conv_size2 + 1;
const int pool_layers_window = 2;
const int pool_layers_stride = 2;
const int imageSizeX_afterPooling = (imageSizeX_afterConvolution - pool_layers_window) / pool_layers_stride + 1; 
...
const int num_lastLayer_inputNeurons = num_finalImages * (imageSizeX - num_conv_layers * (conv_size1 - 1)) * (imageSizeY - num_conv_layers * (conv_size1 - 1));
